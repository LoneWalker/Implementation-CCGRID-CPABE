...
def startAll(ID):
    ID.StartBenchmark([CpuTime, RealTime, NativeTime, Add, Sub, Mul, Div, Exp, Pair, Granular])
    #ID.StartBenchmark([RealTime, Exp, Pair])
...
def getRes(ID, header, footer):
    ...
    result1 = ID.GetGeneralBenchmarks()
    result2 = ID.GetGranularBenchmarks()
    ...
    # res += "\u2502                             \u2502\n"
    res += "\u2502" + f("ZR:  ") + f(result2[Add][ZR] + result2[Sub][ZR]) + f(result2[Mul][ZR] + result2[Div][ZR]) + f(
        result2[Exp][ZR]) + "         \u2502\n"
    res += "\u2502" + f("G1:  ") + f(result2[Mul][G1] + result2[Div][G1]) + f(result2[Exp][G1]) + f(
        result2[Add][G1] + result2[Sub][G1]) + "         \u2502\n"
    res += "\u2502" + f("G2:  ") + f(result2[Mul][G2] + result2[Div][G2]) + f(result2[Exp][G2]) + f(
        result2[Add][G2] + result2[Sub][G2]) + "         \u2502\n"
    res += "\u2502" + f("GT:  ") + f(result2[Mul][GT] + result2[Div][GT]) + f(result2[Exp][GT]) + f(
        result2[Add][GT] + result2[Sub][GT]) + "         \u2502\n"
